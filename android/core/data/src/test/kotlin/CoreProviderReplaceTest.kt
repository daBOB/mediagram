...
class CoreProviderReplaceTest {
    ...
    private fun provider(settings: InMemoryTelegramSettings, answers: (TelegramCredentials) -> Boolean) =
        StoredCoreProvider(settings) { credentials ->
            log += "build ${credentials.apiId}"
            val core = FakeCore(
                account = if (answers(credentials)) {
    ...
    @Test
    fun theOldCoreIsClosedBeforeTheNewOneIsBuiltAndTheNewIdentityKept() = runTest {
     ...
     */
    @Test
    fun anIdentityTelegramRefusesIsClosedAndThePreviousOneComesBack() = runTest {
        val settings = InMemoryTelegramSettings().apply { write(1, OLD_HASH) }
