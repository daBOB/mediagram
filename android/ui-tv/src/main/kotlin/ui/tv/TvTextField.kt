 ...
 *
 * Up and Down leave the field for whatever lies above or below it. The
 * text field only does that itself for a D-pad on a physical input
 * device, and a remote's keys can arrive as a virtual one — then Down,
 * with the keyboard put away, would never reach the results or the next
 * question. A single line has nowhere else for Up or Down to go.
 */
@Composable
