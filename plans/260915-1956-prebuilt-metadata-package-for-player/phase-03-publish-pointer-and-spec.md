...
Built test-first: 10 tests for completing the pointer and running the
publish command, then the implementation, then the documentation.

**Signing was not implemented.** The question was put to the user twice and
answered both times with "implement next phase", so this follows the
standing recommendation: format 1 ships unsigned, and §6 of the spec states
the limitation plainly rather than leaving a reader to discover it. The
`format` field makes adding a signature a version bump rather than a break.

Verified end to end against real published output:
- The archive is uploaded before `latest.json`; a stand-in command recorded
  the order.
- A failing upload exits non-zero and the pointer is never published, which
  is the bug argv-instead-of-shell exists to prevent.
- Walking the documented reader algorithm as a player would — checking
  `format`, `key_id`, size and digest, rebuilding the associated data from
  the pointer and decrypting — yields the manifest and index, and the
  manifest's `created_at` matches the pointer's.
- Editing one authenticated field of the pointer makes decryption fail.

The spec's examples are generated by a test that asserts them against the
document. It immediately earned its place: the key fingerprint in the first
draft was invented rather than computed, and the test failed until the
document matched real output.

Also closed here: `package_key`, `publish_cmd` and `publish_base_url` are
documented in `config.example.toml` and the README, which the phase 2 review
flagged as missing.

418 tests pass, 1 ignored; clippy and rustfmt clean.
