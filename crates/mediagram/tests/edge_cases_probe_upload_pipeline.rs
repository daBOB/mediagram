//! Comprehensive edge case probes for phase 5: streaming part upload,
...
// ===== FakeTransport with failure injection =====
...
struct FakeTransport {
    ...
    fail_on_part: Mutex<Option<u32>>, // Inject failure on specific part index
}
...
impl FakeTransport {
    ...
    fn set_fail_on_part(&self, idx: u32) {
...
impl Transport for FakeTransport {
    async fn send_part(
        ...
        // Check if we should fail on this part (must drop lock before await)
        let should_fail = self
            .fail_on_part
            .lock()
            .unwrap()
            .map(|idx| idx == caption.part.i)
            .unwrap_or(false);
