...
fn a_snapshot_one_schema_behind_is_still_read() {
    ...
    const {
        assert!(
            OLDEST_READABLE_SCHEMA < SCHEMA_VERSION,
            "the floor is below the current schema"
        )
    };
...
fn a_snapshot_below_the_floor_is_refused_and_says_why() {
    ...
    let err = db::open_snapshot(&path)
        .expect_err("too old to read")
        .to_string();
    assert!(
        err.contains(&format!("expects v{OLDEST_READABLE_SCHEMA}")),
        "{err}"
    );
}
