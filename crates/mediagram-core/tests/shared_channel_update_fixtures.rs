...
fn classify_fixtures() {
    let Some(cases) = load::<ClassifyCase>("classify.json") else {
        return;
    };
    for case in cases {
...
fn debounce_fixtures() {
    let Some(cases) = load::<DebounceCase>("debounce.json") else {
        return;
    };
    for case in cases {
        ...
        for step in case.steps {
            match step {
                ...
                Step::Take { take, expect } => {
                    assert_eq!(debouncer.take(take), expect, "{}", case.name)
                }
            }
        }
    }
