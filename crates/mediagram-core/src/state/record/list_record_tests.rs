...
fn a_list_row_reads_removed_only_when_it_is_literally_true() {
    assert_eq!(
        ...
        Some(ListRow { set_id: "01A".into(), updated_at: 1.0, removed: false })
    );
    assert_eq!(
        list_row(&json!({ "setId": "01A", "updatedAt": 1, "removed": true })),
        Some(ListRow { set_id: "01A".into(), updated_at: 1.0, removed: true })
    );
    assert_eq!(
        list_row(&json!({ "setId": "01A", "updatedAt": 1, "removed": "true" })),
        Some(ListRow { set_id: "01A".into(), updated_at: 1.0, removed: false })
...
fn a_collection_names_length_from_another_devices_document_is_capped() {
    ...
    let row = collection_row(&json!({ "id": "c1", "name": format!("  {long}  "), "items": [], "updatedAt": 1 }))
        .unwrap();
...
fn a_collection_with_only_whitespace_for_a_name_is_dropped() {
    assert_eq!(collection_row(&json!({ "id": "c1", "name": "   ", "items": [], "updatedAt": 1 })), None);
}
