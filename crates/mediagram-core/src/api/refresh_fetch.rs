...
pub(super) fn check_manifest(dir: &Path, pointer: &LatestPointer) -> Result<(), CoreError> {
    let text = std::fs::read_to_string(dir.join(catalog::MANIFEST_FILE))
...
mod tests {
    ...
    fn a_manifest_agreeing_with_the_pointer_passes() {
        ...
        std::fs::write(
            dir.path().join(catalog::MANIFEST_FILE),
