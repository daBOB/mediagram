...
pub(in crate::api) struct LibraryEntry {
...
impl LibraryEntry {
    ...
    pub(in crate::api) fn peer(&self) -> Option<PeerRef> {
...
pub(in crate::api) type Handles = BTreeMap<String, LibraryEntry>;

pub(in crate::api) fn path(core: &Core) -> PathBuf {
...
pub(in crate::api) fn read(path: &Path) -> Result<Handles, CoreError> {
...
pub(in crate::api) fn write(path: &Path, handles: &Handles) -> Result<(), CoreError> {
...
pub(in crate::api) fn peer_for_chat(handles: &Handles, chat: i64) -> Option<PeerRef> {
    handles
        .values()
