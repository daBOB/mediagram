...
try {
  // None of these four depends on another: the link, the kids marks and the
  // catalog are all facts about the library itself, and the profile list is
  // asked here only to learn who there is to choose from. Only choosing one
  // waits on that list; everything else runs alongside it.
  ...
  // everything — what has arrived since. Replaced rather than pushed, so a
  // bare load gets this one render rather than this one and then the
  // `hashchange` handler's, and the back button leaves the way it arrived
  // rather than landing on a phantom Movies entry.
