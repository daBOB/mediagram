 ...
 * `credits` and `franchises` tables, and v10 only the wholly new `artwork`
 * table — every one of which every reader treats as optional. Keeping v6
 * readable is what lets the player follow a channel whose uploader has not
 * been upgraded yet: refusing its snapshots would freeze the shelf until
 * someone upgraded another machine.
 */
