...
test("startup asks the link, kids marks, catalog and profile list at once, and state only once a profile is known", async () => {
  // Each of the four calls a real `fetch` synchronously as its first step, so
  // four requests issued without waiting on one another land here as the
  // first four entries, in whatever order `Promise.all` listed them — a
  ...
  await start();
  expect(requested.slice(0, 4).sort()).toEqual(["/api/kids", "/api/player", "/api/profiles", "/api/sets"]);
  const stateIndex = requested.findIndex((url) => url.endsWith("/state"));
  expect(stateIndex).toBeGreaterThanOrEqual(4);
});
